...
class PCA9685:
    ...
    def __init__(self, bus_num: int = 11, address: int = 0x40, frequency: int = 50):
    ...
    def disable_channel(self, channel: int) -> None:
        """Disable one PCA9685 output using the channel's full-off bit."""
        self.bus.write_byte_data(self.address, PCA9685_LED0_ON_L + 4 * channel, 0x00)
        self.bus.write_byte_data(self.address, PCA9685_LED0_ON_H + 4 * channel, 0x00)
        self.bus.write_byte_data(self.address, PCA9685_LED0_OFF_L + 4 * channel, 0x00)
        self.bus.write_byte_data(self.address, PCA9685_LED0_OFF_H + 4 * channel, 0x10)
...
class ServoController:
    ...
    def __init__(self, initialize_esc: bool = True):
        self.pca = PCA9685(bus_num=11, address=0x40, frequency=50)
    ...
    def initialize_outputs(self) -> None:
        """Initialize every output to a stationary state."""
        logger.info("Initializing ESCs and servos to neutral")
        ...
        logger.info("ESCs and servos initialized at neutral")
    ...
    def emergency_stop(self, hold_seconds: float = 1.0, disable_motor_pwm: bool = True) -> None:
        """Hold neutral long enough for the ESC, then optionally disable motor PWM."""
        deadline = time.monotonic() + max(0.0, float(hold_seconds))
        while True:
            self.stop()
            self.reset_auxiliary_outputs()
            if time.monotonic() >= deadline:
                break
            time.sleep(0.05)
        if disable_motor_pwm:
            self.pca.disable_channel(CHANNEL_THROTTLE)
            self.pca.disable_channel(CHANNEL_LIFT_SPEED)
...
def emergency_stop_car(hold_seconds: float = 1.0, disable_motor_pwm: bool = True) -> None:
    get_controller().emergency_stop(hold_seconds=hold_seconds, disable_motor_pwm=disable_motor_pwm)
...
def _emergency_stop_at_exit() -> None:
    if _controller is None:
        return
    try:
        _controller.emergency_stop(hold_seconds=0.25)
    except Exception as exc:
        logger.error("Emergency stop during interpreter shutdown failed: %s", exc)


atexit.register(_emergency_stop_at_exit)
